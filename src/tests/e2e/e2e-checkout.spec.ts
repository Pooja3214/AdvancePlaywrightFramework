...
//{datagen} named import and {} is default import
...
//visual step is used if we want to take screenshot of every page

         // Step 2 — inventory
         await visualStep(page, 'Go to inventory page', async()=>{
            log.info('Step2: Navigate to inventory page ');
            await inventoryPage.open();
         });

           // Step 3 — add one item
           await visualStep(page, 'Add one item to cart', async()=>{
           ...
           await visualStep(page, 'Open the cart', async()=>{
            ...
            await cartPage.open();
            expect(cartPage.rowCount()).toBe(1);
           });
